...
if %ERRORLEVEL% EQU 0 (
  ...
  async installAudioCommand(): Promise<void> {
    ...
    console.log(chalk.gray('   Use "audio enable" or "audio disable" in Claude Code'));
  }
  ...
  private generateAudioCommand(): string {
    ...
    if (isWindows) {
      return `@echo off
REM Audio enable/disable command for Claude Code
...
echo Usage: audio [enable^|disable]
echo.
echo   enable  - Enable audio notifications
echo   disable - Disable audio notifications
exit /b 1`;
    } else {
      return `#!/bin/sh
# Audio enable/disable command for Claude Code
