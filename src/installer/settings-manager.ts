...
if %ERRORLEVEL% EQU 0 (
  ...
  async installAudioCommand(): Promise<void> {
    ...
    console.log(
      chalk.gray('   Use "audio enable", "audio disable", or "audio status" in Claude Code')
    );
  }
  ...
  private generateAudioCommand(): string {
    ...
    if (isWindows) {
      return `@echo off
REM Audio enable/disable/status command for Claude Code
...
exit /b 1`;
      ...
      return `#!/bin/sh
# Audio enable/disable/status command for Claude Code
